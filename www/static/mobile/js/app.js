...
function _ajx_post() {
    ...
    //图片列表页
    $(document).on("pageInit","#list_picture",function (e,id,page) {
        //无限加载
        var loading = false;
        var itemsPerLoad = 10;
        var lastIndex = $("#list_picture .list-container a.col-50").length;
        if(lastIndex < itemsPerLoad){
            $(".infinite-scroll-preloader").remove();
            return;
        }
        function addItems(data) {
            ...
            $('#list_picture .list-container').append(html);
        }
        $("#list_picture .infinite-scroll").on('infinite',function () {
            //如果正在加载，退出
            if(loading) return;
            //设置flag
            loading = true;
            ...
            $.ajax({
                ...
                success:function (data) {
                    ...
                    addItems(data.data);
                    console.log(data.data);
                    lastIndex = $("#list_picture .list-container a.col-50").length;
                }
            })
        })
    })
