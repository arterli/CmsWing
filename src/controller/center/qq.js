...
module.exports = class extends Center {
  ...
  indexAction() {
    ...
    if (this.config('setup.IS_QQ_LOGIN') == 0) {
  ...
  async loginresultAction() {
    ...
    const res = this.get();
    const qq = this.service('qqapi', res.access_token, res.openid);
    const userinfo = await qq.get_user_info();
    const qq_user = await this.model('qq_user').where({openid: res.openid}).find();
    if (think.isEmpty(qq_user)) {
      userinfo.openid = res.openid;
      // think.log(userinfo);
      await this.model('qq_user').add(userinfo);
      return this.redirect(`/center/qq/qqlogin/id/${res.openid}`);
    } else {
  ...
  async qqloginAction() {
    ...
    const qq_user = await this.model('qq_user').where({openid: openid}).find();
  ...
  async logonbindingAction() {
    ...
    if (res.uid > 0) {
      ...
      const qq_info = await this.model('qq_user').where({openid: data.openid}).find();
