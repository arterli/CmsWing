...
module.exports = class extends Center {
  ...
  getuserinfoAction() {
    ...
    const cy_appkey = this.config('setup.CY_APPKEY');
    let ret;
    if (this.is_login) {
      ret = {
  ...
  async loginAction() {
    ...
    const cy_user = await this.model('cy_user').where({cy_user_id: get.cy_user_id}).find();
    if (think.isEmpty(cy_user)) {
      await this.model('cy_user').add(get);
    }
    const uid = await this.model('cy_user').where({cy_user_id: get.cy_user_id}).getField('uid', true);
    console.log(uid);
    // return false;
    let ret;
    if (!this.is_login) {
