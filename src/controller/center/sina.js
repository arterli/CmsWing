...
module.exports = class extends Center {
  ...
  async indexAction() {
    ...
    const redirectURI = `${http_}${this.ctx.host}/center/sina/index`;
    ...
    if (think.isEmpty(this.get('code'))) {
      ...
      const sina = this.service('sina', code, redirectURI);
      const token = await sina.gettoken();
      console.log(token);
      const userinfo = await sina.getuserinfo(token.access_token, token.uid);
      const sina_user = await this.model('sina_user').find(userinfo.id);
  ...
  async loginAction() {
    ...
    const sina_user = await this.model('sina_user').find(id);
    console.log(sina_user);
  ...
  async logonbindingAction() {
    ...
    if (res.uid > 0) {
      ...
      const sina_info = await this.model('sina_user').where({id: data.openid}).find();
