...
module.exports = class extends Center {
    ...
    async oauthAction(){
        ...
        if(this.isweixin && think.isEmpty(openid)){
            ...
            let oauthUrl = pingpp.wxPubOauth.createOauthUrlForCode(this.config('setup.wx_AppID'), `http://${this.ctx.host}/center/weixin/getopenid?showwxpaytitle=1`);
    ...
    async getopenidAction(){
        ...
        let userinfo = await this.api.getUser(openid);
        ...
        if(think.isEmpty(wx_user)){
            ...
            if(think.isEmpty(wx_user.uid)){
                ...
                //更新微信头像
                let filePath=think.resource + '/upload/avatar/' + wx_user.uid;
