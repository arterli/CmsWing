...
module.exports = class extends think.Model {
  ...
  async checkTableExist(model_id) {
    ...
    const res = await think.model('mysql', think.config('db')).query(`SHOW TABLES LIKE '${table_name}'`);
    return res.length;
  }
  ...
  async addtable(mod_id) {
    ...
    const res = await think.model('mysql', think.config('db')).execute(sql);
    return res >= 0;
  }
  /**
     * 新建表字段
  ...
  async updateField(_field) {
    ...
    // 获取默认值
    const def = _field.value != '' ? ' DEFAULT ' + _field.value : '';
