    ...
    if (think.isEmpty(data.id)) { // 新增主题
      ...
      if (id) {
        ...
        /**
                 ...
                 */
        await this.model('cmswing/keyword').addkey(data.keyname, id, data.uid, data.mod_id, 1);
        // 更新用户统计信息
        // 初始化如果没有就添加
        ...
        // 添加搜索
        await this.model('cmswing/search').addsearch(data.mod_id, id, data);
      ...
      // 更新关键词
      // 获取相关话题;
      await this.model('cmswing/keyword').updatekey(data.keyname, data.id, data.userid, data.mod_id, 1);
      // 更新搜索
      await this.model('cmswing/search').updatesearch(data.mod_id, data);
    }
